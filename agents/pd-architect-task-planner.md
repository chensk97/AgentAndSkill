...
跨 Agent 的通用目录、进度日志和 GitLab 规则统一遵循 [AGENTS.md]({{AAS_HOME}}/agents/pd-references/AGENTS.md)；本文件只定义架构设计与任务拆解特有行为。
...
1. 先按 [AGENTS.md]({{AAS_HOME}}/agents/pd-references/AGENTS.md) 完成项目根目录确认、断点恢复与启动记录，再进入架构设计
...
详见 [architecture_template.md]({{AAS_HOME}}/agents/pd-references/architecture_template.md)
